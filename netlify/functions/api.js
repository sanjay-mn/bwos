...
function getDb() {
  ...
  if (!url) throw new Error("NEON_DATABASE_URL env var is not set. See NETLIFY-DEPLOY.md for setup.");
  _db = neon(url);
  return _db;
}
...
async function getProjectDocuments(projectId) {
  ...
  return rows.map(r=>({...r,fileUrl:`/project-files/${r.id}`}));
}
...
exports.handler = async (event, context) => {
  ...
  // Strip the /.netlify/functions/api prefix if present (shouldn't be after redirects but safety)
  ...
  if (method==="GET" && pathname==="/documents/inventory-report") {
    if (!auth||!requirePermission(auth,"reports.view")) return jsonResponse(403,{error:"Forbidden"});
  ...
  const docPoMatch = pathname.match(/^\/documents\/purchase-order\/([^/]+)$/);
  ...
  const docInvMatch = pathname.match(/^\/documents\/invoice\/([^/]+)$/);
  ...
  const docGrnMatch = pathname.match(/^\/documents\/grn\/([^/]+)$/);
  ...
  const docDcMatch = pathname.match(/^\/documents\/delivery-challan\/([^/]+)$/);
  ...
  const docPkgMatch = pathname.match(/^\/documents\/packaging-document\/([^/]+)$/);
  ...
  const docQeMatch = pathname.match(/^\/documents\/quote-estimate\/([^/]+)$/);
  ...
  const docDfmMatch = pathname.match(/^\/documents\/quote-dfm\/([^/]+)$/);
  ...
  const pfMatch = pathname.match(/^\/project-files\/([^/]+)$/);
  ...
  const qfMatch = pathname.match(/^\/quote-files\/([^/]+)$/);
